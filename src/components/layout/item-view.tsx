    ...
    <>
            ...
            {data.genres && (
              <div className="flex gap-2 flex-wrap">
                {data.genres.map((item) => (
                  <span
                    key={item.id}
                    className="bg-dark-accent border border-text-muted px-3 py-1 rounded-full whitespace-nowrap text-text-secondary"
                  >
                    {item.name}
                  </span>
                ))}
                ...
                {casts.map((item) => (
                  <div key={item.id} className="flex flex-col items-center">
                    <Image
                      ...
                      height={225}
                    />
                    <p className="text-center text-text-primary">{item.name}</p>
      ...
      {trailerModalOpened && (
        <div
          ...
          className="fixed top-0 left-0 z-[60] w-screen h-screen flex justify-center items-center bg-black bg-opacity-80 backdrop-blur-sm"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="w-full max-h-screen max-w-xl flex flex-col gap-3 items-start overflow-y-auto bg-dark-lighten border border-dark-accent p-5 rounded-lg shadow-2xl"
          >
